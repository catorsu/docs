...
Clears all information stored in the `mockFn.mock.calls`, `mockFn.mock.instances`, `mockFn.mock.contexts` and `mockFn.mock.results` arrays. Often this is useful when you want to clean up a mock's usage data between two assertions.
...
const mockedSong = jest.mocked(song); // or through `jest.Mocked<Source>`
